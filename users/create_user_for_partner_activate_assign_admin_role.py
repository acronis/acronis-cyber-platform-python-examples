...
if response.ok and response.status_code == 204:
    ...
    if response.ok:
        with open(os.path.join(base_path, 'partner_user.json'), 'w') as outfile:
            json.dump(response.json(), outfile)

        new_user = User(os.path.join(base_path, 'partner_user.json'))
        ...
        if response.ok:
            ...
            if response.ok:
                print(f"User {login} is assigned a partner admin role.")
            else:
