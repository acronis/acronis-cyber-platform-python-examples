...
if response.ok:
    ...
    if response.ok:
        ...
        if response.ok:
            ...
            response = acronis.get(
                f'api/2/tenants/{new_customer.tenant_id}/edition?application_id={cfg.cyber_protection_application_id}&target_edition={cfg.switch_edition}'
            )

            if response.ok:

                edition_switch = {
                   "application_id": f"{cfg.cyber_protection_application_id}",
                    "target_edition": f"{cfg.switch_edition}"
                }

                response = acronis.put(
                    f'api/2/tenants/{new_customer.tenant_id}/edition',
                    data=json.dumps(edition_switch)
                )
